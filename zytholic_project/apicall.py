...
def get_most_similar_beers(name, n_beers=5, similarity="cosine"):
    ...
    if similarity == "cosine":
        ...
        with open("assets/linear.pkl", "rb") as file:
            kernel = pickle.load(file)
    results = get_recommendations(working_df, name, sim_matrix=kernel, n_recomm=n_beers)
    results = results[["name", "brewery", "style", "abv", "min ibu", "max ibu"]]

    return results.to_dict()


def get_most_similar_beers_ibu_abv(
    name, ibu, abv, input_country=None, n_beers=5, similarity="cosine"
):
    """
    ...
    results = results[
        ["name", "brewery", "style", "abv", "min ibu", "max ibu", "country"]
    ]
...
def get_similar_style(
    style, abv=None, ibu=None, input_country=None, n_beers=5, similarity="cosine"
):
    """
    ...
    score = sorted(
        list(enumerate(kernel[0])), key=lambda x: x[1], reverse=True  # Verify here
    )
