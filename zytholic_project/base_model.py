...
class BaseModel:
    ...
    def get_data(self):
        ...
        dfbrewb = pd.merge(
            dfbeer, dfbrew[["brewery_id", "brewery"]], how="left", on=["brewery_id"]
        )
        dftopbrew = pd.merge(
            dftop,
        ...
        working_df = dftopbrew.drop(
            ["description", "key", "style key"], axis=1
        ).drop_duplicates()
    ...
    def reformat_styles(self, working_df, ohe=True):
        ...
        if ohe:
            for feat in features_to_implement:
                working_df[feat] = [
                    1 if feat in elm.lower() else 0 for elm in working_df["style"]
                ]

        working_df.rename(
            columns={"style": "original_style", "simple_style": "style"}, inplace=True
        )
        return working_df
    ...
    def set_preprocess_pipeline(self):
        ...
        pipe_style_country = make_pipeline(
    ...
    def train_test_process_data(self):
        ...
        X_train, X_test = train_test_split(
            self.working_df, test_size=0.25, stratify=self.working_df["style"]
        )
